var app = angular.module('weUploadModel', ['toastr','ipCookie']);
app.controller('weUploadCtrl', function($scope, weSer,$stateParams,$state,toastr,$http,ipCookie){
    $scope.files = [];
    $scope.affirmFile = [];
    var oldFiles = [];
    //删除文件
    ...
    $scope.updataSel = function () {
        ...
        if (_files.length) {
            ...
            fd.append('user', ipCookie('username'));
            $http({
                method: 'POST',
                url: '/we/upload',
                headers: {
                ...
                if (response.data.code == 200) {
                    ...
                    $state.go('root.database.we.list[12]',{id:$stateParams.id,tonken:$stateParams.tonken});
                    $scope.files = [];//预览的数组
